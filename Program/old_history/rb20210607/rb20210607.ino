...
#define XSHUT_TOF_5  6
#define XSHUT_TOF_6  7
...
void dislplay_print_4_tofs(int tof1_distance, int tof2_distance, int tof3_distance, int tof4_distance, int tof5_distance, int tof6_distance) {
  ...
  display.print(tof5_distance);
  display.setCursor(25, 20);
  display.print(tof6_distance);
  display.display();
  //delay(100);
...
void loop() {
  ...
  if (0 != (tof_5.readReg(RESULT_INTERRUPT_STATUS) & 0x07)) {
    tof5_distance = tof_5.readReg16Bit(RESULT_RANGE_STATUS + 10);
    tof_5.writeReg(SYSTEM_INTERRUPT_CLEAR, 0x01);
  }
  
  if (0 != (tof_6.readReg(RESULT_INTERRUPT_STATUS) & 0x07)) {
    tof6_distance = tof_6.readReg16Bit(RESULT_RANGE_STATUS + 10);
    tof_6.writeReg(SYSTEM_INTERRUPT_CLEAR, 0x01);
  }

  dislplay_print_4_tofs(tof1_distance, tof2_distance, tof3_distance, tof4_distance, tof5_distance, tof6_distance);
  ...
  err = ((tof3_distance - tof2_distance) + (tof4_distance - tof1_distance)) / 2;
  ...
  if ((sr < 200) or (tof2_distance < 100) or (tof2_distance < 100) or (tof5_distance < 100) or (tof6_distance < 100)){
    if (tof4_distance < tof1_distance){
      servo.write(130);}
    else {
      servo.write(40);
    }
...
bool setup_4_tofs(int timeout) {
  ...
  pinMode(XSHUT_TOF_5, OUTPUT);
  //  pinMode(XSHUT_TOF_6, OUTPUT);

  Wire.begin();   // активируем интерфейс I2C, arduino будет мастером

  tof_6.setAddress(TOF_6_ADDR);     // задаем адрес для текущего активного модуля (обязательно должен отличаться от 0x29)
  
  pinMode(XSHUT_TOF_5, INPUT);      // активируем следующий модуль (у него по умолчанию адрес 0х29, поэтому не забываем его сменить)
  delay(10); 
  tof_5.setAddress(TOF_5_ADDR);     // задаем адрес для текущего активного модуля (обязательно должен отличаться от 0x29)
  
  pinMode(XSHUT_TOF_4, INPUT);      // активируем следующий модуль (у него по умолчанию адрес 0х29, поэтому не забываем его сменить)
  delay(10); 
  ...
  if (tof_6.init() == 0) {
    Serial.println("Не удалось обнаружить и инициализировать tof-модуль 6!");
    return 1;
  }
