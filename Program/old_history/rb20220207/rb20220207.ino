...
void loop() {
  ...
  while (1) {
    ...
    //      Serial.println(sp);
    speed_control();
    if (r_90 - r_45*sin(pi/2) < -(l_90 - l_45*sin(pi/2)))
      err = r_90 - r_45*math.sin(45);
    else
      err = -(l_90 - l_45*math.sin(pi/2));
    serial_print();
    //display_print();
